...
After refiling, save the org buffers because I often forget to do so.
Especially inbox.org.
...
Ask to save archive file when it is modified.
...
(use-package shell-pop
  :bind
  (("C-t" . shell-pop)) ; C-t is bound to transpose-chars by default, which I never use.

  ; custom.el says "init file should contain only one such instance (of custom-set-variables).
  ; If there is more than one, they won't work right.
  ;
  ; :config
  ; ;; The use of custom-set-variables is needed because of a quirk in shell-pop.
