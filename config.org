...
Where Emacs stores and finds various files.
...
Some packages installed in emacs-d-directory/elisp/ are single files
while others are placed inside their own sub-directories.  Prepend
emacs-d-directory/elisp/ and all of it's subdirectories to load-path.
...
Default to filename at point for C-x C-f.  I tried ido-mode, but I
don't like the M-p, M-n behaviour.  See
...
Make two buffers with the same file name open distinguishable.  See
[[http://www.emacswiki.org/emacs/uniquify][emacswiki Uniquify]].
...
** Jump between other symbols found at point with M-n and M-p
...
*** Display and cleanup bogus whitespace
  *
Se*e http://www.emacswiki.org/emacs/WhiteSpace
  *
#+*BEGIN_SRC emacs-lisp
...
Th*is should not be necessary because most modes should set
re*quire-final-newline to mode-require-final-newline, but most do not
(E*macs-Lisp for one).  The risk here is if we open a binary file we
mi*ght append a newline.
  *
#+*BEGIN_SRC emacs-lisp
...
WebJump is a programmable Web hotlist (or bookmark) facility that uses
Emacs completion.  Jump to a Web site from a programmable hotlist.
